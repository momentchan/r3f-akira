// Procedural vein generator. Both this and fit-veins-to-uv write tujlip-veins.png;
// use fit when hand-drawn source art is available.
import path from 'path';
import { fileURLToPath } from 'url';
import { PNG } from 'pngjs';
import { buildPetalMask, readPng, writePng } from './lib/pngio.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const rootDir = path.resolve(__dirname, '../..');

const INPUT_PATH = path.join(rootDir, 'scripts/textures/input/tujlip.png');
...
function generateVeinMap(source) {
  ...
  const mask = buildPetalMask(data, width, height, CONFIG.backgroundThreshold);
